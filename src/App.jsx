...
function NoteListView({ notes, error, onAddClick, loaded }) {
  ...
  const dateNotes = notes.filter((note) => formatDateKey(note.createdAt) === selectedDate)
  const dateNoteCount = dateNotes.length
  const visibleNotes = dateNotes.slice(0, MAX_VISIBLE_NOTES)
